...
export const UpdateForm = ({ user }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    // API LINK FOR USERS BELOW DOES NOT WORK, NEEDS FIXING!
    console.log(data);
    const updateUser = await fetch(
      `https://reman-backend.onrender.com/users/${user.Username}`,
      {
        method: 'PUT',
  ...
  //AGAIN API BELOW DOES NOT WORK
  const handleDeregister = () => {
    fetch(`https://reman-backend.onrender.com/users/${user.Username}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
